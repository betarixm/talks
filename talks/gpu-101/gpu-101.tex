...
\begin{frame}{An SM keeps execution and its working state close}
  ...
  각 스케줄러는 담당 warp들의 준비된 명령을 골라 실행 자원에 공급합니다. 스케줄러 네 개가 warp 네 개를 뜻하지는 않습니다. 아래의 64 KB 레지스터 파일은 thread들의 작업값을 보관합니다. 진한 초록 칸 32개는 FP32 또는 INT32 연산을 하는 유닛으로, 한 사이클에 둘 중 한 종류를 처리합니다. 구획마다 Tensor Core 하나와 load/store 유닛 네 개도 있습니다.
...
\begin{frame}{A current example: the NVIDIA Rubin GPU}
  \begin{diagram}{4.8}
    \draw[hwbox] (0,.25) rectangle (10.1,4.7);
    \node[anchor=west,font=\small\bfseries,text=hw] at (.25,4.4) {One GPU package · two compute dies};
    \foreach \d in {0,1} {
      \pgfmathsetmacro{\dx}{.3+5.05*\d}
      \draw[hwbox,fill=white] (\dx,1.25) rectangle (\dx+4.4,4.1);
      \node[font=\small\bfseries,text=hw] at (\dx+2.2,3.8) {Compute die \d};
      \foreach \x in {0,...,3} \foreach \y in {0,1}
        \node[hwbox,minimum width=.85cm,minimum height=.5cm,font=\scriptsize] at (\dx+.62+1.05*\x,3.2-.7*\y) {SM};
      \node[membox,minimum width=3.9cm,minimum height=.45cm,font=\scriptsize] at (\dx+2.2,1.75) {L2 cache / memory controllers};
    }
    \draw[flow,<->] (4.7,2.8)--(5.35,2.8);
    \node[lab] at (5.05,.75) {NV-HBI connects the dies · SM count drawn is illustrative};
    \node[anchor=west,font=\Large\bfseries,text=hw] at (10.55,4.05) {224 SMs};
    \node[anchor=west,lab] at (10.55,3.52) {Published GPU configuration};
    \node[membox,minimum width=3.8cm,minimum height=1.5cm,font=\small] at (12.5,2.35)
      {\textbf{HBM4 memory}\\[.12cm]Up to 288 GB\\Up to 22 TB/s};
    \draw[flow,<->] (10.1,2.35)--(10.6,2.35);
    \node[anchor=west,font=\scriptsize,text=muted,align=left] at (10.55,.8)
      {Published as of 2026-09-10\\Preliminary specifications};
  \end{diagram}
  \takeaway{Even a two-die GPU connects many SMs to a shared memory system.}
  \credit{https://developer.nvidia.com/blog/inside-nvidia-rubin-gpu-architecture-powering-the-era-of-agentic-ai/}{NVIDIA · Inside Rubin GPU Architecture · July 2026 · simplified grouping}
  \note{여기서는 최신 공개 세대인 NVIDIA Rubin GPU의 전체 구성을 보겠습니다. 앞에서 SM 내부를 본 RTX Blackwell과는 다른 제품입니다. NVIDIA가 공개한 Rubin 구성에는 SM 224개가 있고, compute die 두 개를 NV-HBI라는 고속 연결로 묶어 하나의 GPU로 사용합니다. 그림의 SM 상자는 반복 배치를 보여주기 위한 예시이고 실제 개수를 모두 그리지는 않았습니다.

  각 다이에는 SM들과 L2 캐시, 메모리 컨트롤러가 있고, 패키지의 HBM4 메모리가 입력과 출력을 보관합니다. 공개 사양은 GPU당 최대 288 GB 용량과 초당 22 TB 대역폭입니다. 이 수치들은 2026년 9월 10일 기준의 예비 사양입니다. Rubin도 CUDA를 지원하며, 현재 CUDA 13.4에서 sm\_107 대상으로 프리뷰 지원하는 단계입니다.

  이제 GPU 하나가 반드시 실리콘 다이 하나인 것도 아닙니다. 그래도 여러 SM을 메모리 시스템에 연결한다는 앞의 구조를 확인할 수 있습니다. 그러면 이 여러 SM에 일을 나누는 것은 누구의 몫일까요?}
\end{frame}
